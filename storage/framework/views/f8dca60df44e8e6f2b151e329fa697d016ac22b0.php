...
<script>
  ...
  const getlistExam = () => {
    fetch((`<?php echo url('api/questions'); ?>/?page=${currentPage}`))
    .then((response) => {
  ...
  const showListExam = Exams => {
    listExam.innerHTML = "";
        listExam.innerHTML += `
        <h4 class="mb-4 text-capitalize">KATEGORI : ${Exams.questions_categorie_name}</h4>
        <h1 class="mb-4 text-capitalize text-center">${Exams.name}</h1>
       `
      };
    ...
    const sendData = () => {
        ...
        fetch('<?php echo url("api/respondent"); ?>/add', {
            method: 'POST',
            headers: {
            ...
            .then(res => res.json())
            .then(res => window.location.href = '<?php echo e(url("thanks")); ?>');
    }

    const previous = () => {
        if(currentPage === 1) {
            window.location.href = '<?php echo e(url("survey")); ?>'
        }else {
            currentPage--
            getlistExam()
        }
...
<?php echo $__env->make('frontend.layouts.app', \Illuminate\Support\Arr::except(get_defined_vars(), ['__data', '__path']))->render(); ?><?php /**PATH /Users/satrionugroho/Sites/localhost/skm_libas/resources/views/frontend/exams.blade.php ENDPATH**/ ?>