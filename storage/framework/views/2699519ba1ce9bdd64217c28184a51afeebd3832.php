  ...
  <div class="row justify-content-center">
      <div class="col-12 col-md-6 table-responsive">
          <div class="mx-sm-3 mx-lg-4 mb-2" id="select-option">
                <p id="unit"></p>
                <select class="form-select text-capitalize" aria-label="Default select example" id="select">
                </select>
          </div>
          <table class="table table-striped">
              <thead>
                  <tr>
                      <th class="text-capitalize text-start">Unsur</th>
                      <th class="text-capitalize text-center">Mean</th>
                      <th class="text-capitalize text-center">Nilai Penimbang</th>
                      <th class="text-capitalize text-center">SKM</th>
                  </tr>
              </thead>
...
.form-select {
    width: auto;
        ...
        <script>
            ...
            const getlist = () => {
                let link = `<?php echo e(url('api/respondent/result')); ?>/${unitId}/${filterDate}`
                fetch((link))
                .then((response) => {
            ...
            const getunit = () => {
                const linkUnits = <?php echo '"'.url('api/units').'"'; ?>
            ...
            const showListTable = ListTab => {
                ...
                ListTab.forEach(item => {
                    datatable.innerHTML += `
                    <tr>
                        <td class='text-capitalize text-start'>${item.questions_categorie.name}</td>
                        <td class='text-lowercase text-center'>${item.rata_rata}</td>
                        <td class='text-lowercase text-center'>0.111</td>
                        <td class='text-lowercase text-center'>${item.skm}</td>
                    </tr>
                    `
                });
            }
...
<?php echo $__env->make('frontend.layouts.app', \Illuminate\Support\Arr::except(get_defined_vars(), ['__data', '__path']))->render(); ?><?php /**PATH /Users/satrionugroho/Sites/localhost/skm_libas/resources/views/frontend/dashboard.blade.php ENDPATH**/ ?>